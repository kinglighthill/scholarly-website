...
import { AccountCredentialsState, SignupLevelProps } from '../../types/components/reusables/affiliate_signup';
...
export default function AccountCredentials({ formData, updateFormData }: SignupLevelProps) {
  ...
  const comparePasswords = () => {
    if (accountCredentials.password !== accountCredentials.confirm_password) {
    ...
    <form>
      <VStack spacing={6} align="start">
        ...
        <FormControl isRequired>
          ...
          <Input id="phone-number" name="phone_number" value={accountCredentials.phone_number} type="tel" placeholder="Enter phone number" bg="white" fontSize={13}
            _placeholder={{color: "brand.gray"}} focusBorderColor="brand.lime.500" onChange={(e) => updateFormData(e.target.name, e.target.value)}
          />
        </FormControl>
        <FormControl isRequired>
        ...
        <FormControl isRequired isInvalid={!passwordsMatch}>
          ...
          <Input id="confirm-password" name="confirm_password" value={accountCredentials.confirm_password} type="password" placeholder="Confirm password" bg="white" fontSize={13}
