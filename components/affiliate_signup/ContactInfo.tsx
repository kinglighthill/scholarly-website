...
import { ContactInfoState, SignupLevelProps } from '../../types/components/reusables/affiliate_signup';
    ...
    <form style={{textAlign: "center"}}>
      <VStack spacing={6} align="start">
        <FormControl isRequired>
          ...
          <Select id="select-country" name="country_id" value={contactInfo.country_id} aria-label="Select Country" placeholder="Select country" fontSize={13}
        ...
        <FormControl isRequired>
          ...
          <Select id="state-of-origin" name="state_of_origin_id" value={contactInfo.state_of_origin_id} aria-label="Select State of Origin" placeholder="Select state" fontSize={13}
            _focus={{borderColor: "brand.lime.500"}} _focusVisible={{boxShadow: "0 0 0 1px #46AC66", fontSize: 16}}
            onChange={(e) => updateFormData(e.target.name, e.target.value)}
          >
            {states.map(state => (
        ...
        <FormControl isRequired>
          ...
          <Select id="state-of-residence" name="state_of_residence_id" value={contactInfo.state_of_residence_id} aria-label="Select State of Residence" placeholder="Select state" fontSize={13}
            _focus={{borderColor: "brand.lime.500"}} _focusVisible={{boxShadow: "0 0 0 1px #46AC66", fontSize: 16}}
            onChange={(e) => updateFormData(e.target.name, e.target.value)}
          >
            {states.map(state => (
