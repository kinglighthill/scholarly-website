...
import Testimonials, {
  Testimonial,
} from "../components/reusables/Testimonials";
...
export const getStaticProps: GetStaticProps = async () => {
  try {
    ...
    const responses = await Promise.all(
      endpoints.map((endpoint) => fetchContent(endpoint)),
    );
...
const Home: NextPage = (
  ...
  const testimonials = testimonialsData.map(
    (testimonial: TestimonialProps, index: number) => (
      <Testimonial
        key={testimonial.full_name + index + 1}
        full_name={testimonial.full_name}
        user_type={testimonial.user_type}
        rating={testimonial.rating}
        profile_pic_sm={testimonial.profile_pic_sm}
  ...
  return (
    <Page
      title="Scholarly Africa | Improved Learning, Better Academic Performance"
      ...
      keywords={[
        ...
        "Desktop education apps",
        "iOS education Apps",
        "Playstore education apps",
        "Google Education",
        "Activation Pins",
        "Education blog",
        "School",
    ...
    >
      ...
      <Box
        as="section"
        bgColor="brand.nearWhite"
        pt={{ base: "60px", md: "82px" }}
        pb={{ base: "60px", md: "115px" }}
      >
        <Box
          textAlign="center"
          mb={{ base: "90px", md: 12 }}
